...
import { MenuList, NotificationStatusUpdate, StudentAuthApi, StudentDetails } from '../apis/student-api';
...
export default function NotificationScreen() {
    ...
    ////////////////////////////////////////////////////// USE ROUTES //////////////////////////////////////////////////////
    const {params:{ notification_array }} = useRoute();
    const unReadCount = notification_array.filter(noti => noti.status === "UN_READ").length;
    ...
    const [notifications, setNotifications] = useState(notification_array ? notification_array : [])
    const [notiCount, setNotiCount] = useState(unReadCount ? unReadCount : 0)
    //UserDetails
    ...
    useFocusEffect(
        React.useCallback(() => {
            async function fetchData(){
                let token = await SecureStore.getItemAsync('UserAccessToken')
                if (user?._id) {
                    const response = await NotificationStatusUpdate({ student_id: user?._id } ,token)
                    if (response.status == "success" ) {
                ...
                >
                    {
                        ...
                        notifications.map(notification => (
                            <View key={notification._id} className="justify-center mx-3 mt-4 py-3 px-5 bg-gray-200 rounded-2xl">
                                <View className="w-full flex flex-row">
                                    <Text className="font-extrabold text-xl">{notification?.title}</Text>
                                    <Text className="font-extrabold text-2xl ml-auto">{notification?.status === "UN_READ" && "🔹"}</Text>
                                </View>
                                <Text className="font-bold text-md mt-2">{notification?.message}</Text>
                            </View>
                        ))
                        :
                        <View className="items-center bg-white py-40">
                            <Icon.XCircle height="100" width="100" strokeWidth={2} stroke="#b0aeae"/>
                            <Text style={{ fontWeight: '900', color:'#b0aeae' }} className="text-xl">No Notifications</Text>
                        </View>
                    }

                    {/* footer */}
                    <View style={{backgroundColor: "#f5d45d" }} className="justify-center px-4 pt-6 pb-24 items-center mt-4">
